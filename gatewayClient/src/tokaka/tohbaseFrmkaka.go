...
var logFileName = flag.String("log", "consumer-hbase.log", "Log file name")
...
func init() {
	...
	if logErr != nil {
		fmt.Println("Fail to find", *logFile, "consumer-hbase start Failed")
		os.Exit(1)
	}
...
func main() {
	...
	for i := config.MIN_TO_KAKA; i < config.MAX_CONNECT_TO_KAKA; i++ {
		...
		go myConsumer(producer, eq, int64(i))
	}
...
func myConsumer(p sarama.AsyncProducer, eq *eq.Eq, seq int64) {
	...
	for !stopLoop {
		...
		msg.Value = sarama.ByteEncoder(fmt.Sprintf("%s,%d", eq.StrMsg, num))
